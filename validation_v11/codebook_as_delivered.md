...
| `regional_identity_spec` | The reported quantity is a CM subtype fraction (atrial, ventricular, sinoatrial, chamber or heart-field), not a pan-CM yield. The code turns on the reported quantity, not the protocol’s intent, so a pan-CM marker measured under a subtype-directed protocol is still a pan-CM yield. | `subtype spec` |
...
| `no_value_reported` | The arm is in an eligible construct and its purpose is compatible with reporting a yield, but no numerical CM fraction appears in the source. | `no value reported` |
